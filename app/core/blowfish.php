...
class _blowfish
{
    ...
    /** Encryption Block Mode: ECB, CBC actually.
      ...
      * @access private
      */
    var $_blockmode = ENCRYPT_MODE_ECB;
    ...
    /** The Constructor of the class.
      ...
      * @return void
      */
    function _blowfish($blockmode, $key)
    {     
    ...
    function _init()
    {
        ...
        $key  = array_values(unpack("C*",$this->_key));
        $keyl = count($key);
    ...
    function _cbc_encrypt($plain)
    {
        ...
        for($i = 0;$i < $plainsize;$i = $i + $blocksize)
        {
            ...
            $lcipher = $this->_encrypt($block ^ $lcipher);
            $cipher .= $lcipher;
        }
    ...
    function _cbc_decrypt($cipher)
    {
        ...
        for($i = 0;$i < $ciphersize;$i = $i + $blocksize)
        {
            ...
            $dblock  = $lcipher ^ $this->_decrypt($block);
            $lcipher = $block;
...
/**
 * Managers encryption/decryption of the blowfish algorithm
 * @package efusion
 * @subpackage core
 */
class blowfish extends _blowfish
{
	/**
	 * @var string Random salt value used to enhance the blowfish encryption key
	 */
	var $_randomSalt;
	...
	function blowfish($blockmode = ENCRYPT_MODE_ECB, $key = null)
	{
		//TODO: Incorporate this salt value into the key somehow
		$this->_randomSalt = "832sdf934535atwsdg83t6134tgwv4wt";	
        
        parent::_blowfish($blockmode, $key);
	}
	...
	function load($event, $params){
		//Place holder function	
	}
	
	function prepare(&$smarty)
	{

	}
	
	/**
	 * @title refreshSalt method
	 * @description Decrypts a password, and encrypts it with the new key
	 * @param string newSalt A new 16 character salt string (must be random)
	 * @return boolean True for success, else false
	 */
	function refreshSalt($newSalt)
	{
		//On success, set new Salt
		$this->_randomSalt = $newSalt;
	}
    ...
    { 
        ...
        case ENCRYPT_MODE_ECB:
            $cipher = parent::_ecb_encrypt($plain);
            break;
        
        case ENCRYPT_MODE_CBC:
            $cipher = parent::_cbc_encrypt($plain);
            break;
    ...
    {        
        ...
        switch($this->_blockmode) {
        case ENCRYPT_MODE_ECB:
            $plain = parent::_ecb_decrypt($cipher);
            break;
        
        case ENCRYPT_MODE_CBC:
            $plain = parent::_cbc_decrypt($cipher);
            break;
